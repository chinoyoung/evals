  ...
  async cleanupDuplicateUsers(): Promise<{ removed: number; kept: number }> {
    ...
    for (const [email, userGroup] of emailMap) {
      if (userGroup.length > 1) {
        ...
        console.log(`Email ${email}: Keeping ${keepUser.uid}, removing ${duplicateUsers.length} duplicates`);
      } else {
        kept++;
      }
    }
    ...
    if (removed > 0) {
      await batch.commit();
      console.log(`Cleanup complete: Removed ${removed} duplicate users, kept ${kept} users`);
    } else {
...
export const evaluationService = {
  async createEvaluation(evaluationData: Omit<Evaluation, 'id' | 'createdAt'>): Promise<string> {
  ...
  async getUserEvaluations(userId: string, type?: 'pending' | 'completed'): Promise<Evaluation[]> {
    let q = query(
  ...
  async updateEvaluation(id: string, updates: Partial<Evaluation>): Promise<void> {
    const docRef = doc(db, 'evaluations', id);
    
    // Convert Date objects to Timestamps for Firestore
    const firestoreUpdates: Record<string, unknown> = { ...updates };
    if (updates.dueDate) firestoreUpdates.dueDate = convertToTimestamp(updates.dueDate);
    if (updates.completedDate) firestoreUpdates.completedDate = convertToTimestamp(updates.completedDate);
...
export const templateService = {
  async createTemplate(templateData: Omit<EvaluationTemplate, 'id' | 'createdAt'>): Promise<string> {
