...
export default function AdminEvaluationsPage() {
  ...
  const filteredEvaluations = evaluations.filter(evaluation => {
    ...
    const statusMatch = filterStatus === 'all' || evaluation.status === filterStatus;
                        ...
                        <div className="flex-1">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-500">
                            <div className="flex items-center space-x-2">
                              ...
                              <span>
                                <strong>Evaluator:</strong> {evaluator?.displayName || evaluator?.email} ({getRoleDisplayName(evaluator?.role || '')})
                              </span>
                            </div>
                            <div className="flex items-center space-x-2">
                              <Users className="h-4 w-4 text-green-600" />
                              <span>
                                <strong>Evaluatee:</strong> {evaluatee?.displayName || evaluatee?.email} ({getRoleDisplayName(evaluatee?.role || '')})
                              </span>
                            </div>
                            <div className="flex items-center space-x-2">
                              <Calendar className="h-4 w-4 text-purple-600" />
                              <span>
                                <strong>Due:</strong> {evaluation.dueDate.toLocaleDateString()}
                              </span>
                            </div>
                          </div>
                          
                          <div className="mt-3 text-sm text-gray-500">
                            <span><strong>Questions:</strong> {evaluation.questions.length}</span>
                            {evaluation.responses.length > 0 && (
                              <span className="ml-4"><strong>Responses:</strong> {evaluation.responses.length}</span>
                            )}
                          </div>
                        </div>
                      </div>
