...
export default function SmartEvaluatorAssignment({
  ...
  const [evaluateeDepartmentFilter, setEvaluateeDepartmentFilter] = useState<
    string | null
  >(null);
    ...
    if (selectedEvaluator && selectedEvaluatees.length > 0) {
      ...
      if (evaluator && firstEvaluatee) {
        ...
        if (evaluator.uid === firstEvaluatee.uid) {
          ...
          evaluator.role === "manager" &&
          firstEvaluatee.role === "employee"
        ) {
          type = "manager_to_employee";
        } else if (
          evaluator.role === "employee" &&
          firstEvaluatee.role === "manager"
        ) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!evaluationType) {
      alert(
        "Please wait for evaluation type to be detected or select evaluatees first"
      );
      return;
    }
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          >
            <option value="">
              Choose an employee to assign evaluations to...
            </option>
            {users
              .filter(
                (user) =>
        ...
        {selectedEvaluator && (
          <div>
              ...
              <div className="flex flex-wrap gap-2">
                {["All", "Tech", "Content", "Admin", "Sales"].map((dept) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setEvaluateeDepartmentFilter(dept === "All" ? null : dept)
                    }
                    className={`px-3 py-1 text-sm rounded-full border transition-colors ${
                      (dept === "All" && !evaluateeDepartmentFilter) ||
                      evaluateeDepartmentFilter === dept
            ...
            {evaluationType ? (
              <div
                className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${typeInfo.color} mb-3`}
              >
                <TypeIcon className="h-4 w-4 mr-2" />
                {typeInfo.label}
              </div>
            ) : (
              <div className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 mb-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
              {suggestedEvaluatees
                .filter(
                  (user) =>
                    !evaluateeDepartmentFilter ||
                    user.department === evaluateeDepartmentFilter
                )
                .map((user) => (
                  <button
                    key={user.uid}
                    ...
                    onClick={() => {
                      if (selectedEvaluatees.includes(user.uid)) {
                        setSelectedEvaluatees(
                          selectedEvaluatees.filter((id) => id !== user.uid)
                        );
                      } else {
                        setSelectedEvaluatees([
                          ...selectedEvaluatees,
            ...
            <div>
              ...
              <select
                ...
                className="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white min-h-[120px]"
              >
                {users
            ...
            >
              {assigning ? (
                ...
                <>
                  ...
                  <span>
                    Assign {selectedEvaluatees.length} Evaluation
                    {selectedEvaluatees.length > 1 ? "s" : ""}
                  </span>
                </>
              )}
            </button>
      ...
      <div className="mt-6 p-4 bg-gray-50 rounded-lg">
        ...
        <div className="text-sm text-gray-600 space-y-1">
          <p>
            • <strong>Manager to Employee:</strong> Managers evaluate their team
            members
          </p>
          <p>
            • <strong>Employee to Manager:</strong> Team members provide upward
            feedback
          </p>
          <p>
            • <strong>Peer to Peer:</strong> Colleagues at the same level
            evaluate each other
          </p>
          <p>
            • <strong>Admin Review:</strong> Administrators conduct
            comprehensive reviews
          </p>
          <p>
            • <strong>Multiple Assignments:</strong> One person can be assigned
            to evaluate multiple people
          </p>
        </div>
      </div>
    </div>
